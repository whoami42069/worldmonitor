...
export class PanelLayoutManager implements AppModule {
      ...
      <div class="header">
        <div class="header-left">
          <div class="variant-switcher">
            <button class="variant-option active" data-tab="dunya" id="tabDunya">
              <span class="variant-icon">🌍</span>
              <span class="variant-label">DÜNYA</span>
            </button>
            <span class="variant-divider"></span>
            <button class="variant-option" data-tab="mideast" id="tabMideast">
              <span class="variant-icon">⚔️</span>
              <span class="variant-label">ORTA DOĞU</span>
            </button>
          </div>
  ...
  private createPanels(): void {
    ...
    // Middle East War News panel
    const mideastWarPanel = new NewsPanel('mideast-war', 'Orta Doğu Savaş Haberleri');
    this.attachRelatedAssetHandlers(mideastWarPanel);
    this.ctx.newsPanels['mideast-war'] = mideastWarPanel;
    this.ctx.panels['mideast-war'] = mideastWarPanel;
    ...
    for (const k of ['insights', 'strategic-risk', 'cii', 'strategic-posture']) {
      const el = this.ctx.panels[k]?.getElement();
      if (el) el.style.order = '0';
    }

    // Wire up DÜNYA / ORTA DOĞU header tabs
    const tabDunya = document.getElementById('tabDunya');
    const tabMideast = document.getElementById('tabMideast');
    if (tabDunya && tabMideast) {
      tabDunya.addEventListener('click', () => {
        tabDunya.classList.add('active');
        tabMideast.classList.remove('active');
        window.scrollTo({ top: 0, behavior: 'smooth' });
      });
      tabMideast.addEventListener('click', () => {
        tabMideast.classList.add('active');
        tabDunya.classList.remove('active');
        const warPanel = this.ctx.panels['mideast-war']?.getElement();
        if (warPanel) warPanel.scrollIntoView({ behavior: 'smooth', block: 'start' });
      });
    }

    // Show loading state for mideast-war panel
    mideastWarPanel.setContent('<div class="insights-empty">Haberler yükleniyor...</div>');
